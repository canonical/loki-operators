...
"""Helper module for interacting with the Loki configuration."""
    ...
    def __post_init__(self):
        ...
        if not asdict(self).get("s3", "") and not asdict(self).get("s3", ""):
            ...
            raise InvalidConfigurationError(
                "Loki `backend` type must include a configuration block which matches that type"
            )


class LokiBaseConfig(BaseModel):
    """Base class for loki config schema."""
