...
from loki_config import BUCKET_NAME, S3_RELATION_NAME, _S3ConfigData
...
class LokiCoordinatorK8SOperatorCharm(ops.CharmBase):
    ...
    def __init__(self, *args: Any):
        ...
        self.remote_write_provider = PrometheusRemoteWriteProvider(
            charm=self,
            server_url_func=lambda: LokiCoordinatorK8SOperatorCharm.external_url.fget(self),  # type: ignore
        ...
        self.metrics_endpoints = MetricsEndpointProvider(
            ...
            refresh_event=[
                self.on.loki_cluster_relation_joined,
                self.on.loki_cluster_relation_changed,
                self.on.loki_cluster_relation_departed,
                self.on.loki_cluster_relation_broken,
            ],
        )
        ...
        # Loki Cluster
        self.framework.observe(self.on.loki_cluster_relation_joined, self._on_loki_cluster_joined)
        self.framework.observe(
            self.on.loki_cluster_relation_changed, self._on_loki_cluster_changed
        )
        self.framework.observe(
            self.on.loki_cluster_relation_departed, self._on_loki_cluster_changed
        )
        self.framework.observe(self.on.loki_cluster_relation_broken, self._on_loki_cluster_changed)
        # S3 Requirer
    ...
    def _on_loki_cluster_joined(self, _):
        self.nginx.configure_pebble_layer(tls=self._is_tls_ready)
        self._render_workers_alert_rules()
        self._update_loki_cluster()

    def _on_loki_cluster_changed(self, _):
        self.nginx.configure_pebble_layer(tls=self._is_tls_ready)
        self._render_workers_alert_rules()
        self._update_loki_cluster()

    def _on_loki_cluster_departed(self, _):
        self.nginx.configure_pebble_layer(tls=self._is_tls_ready)
        self._render_workers_alert_rules()
        self._update_loki_cluster()
    ...
    def _on_collect_status(self, event: CollectStatusEvent):
        ...
        if not s3_config_data and self.has_multiple_workers():
            event.add_status(
                ops.BlockedStatus(
                    "When multiple units of Loki are deployed, you must add a valid S3 relation. S3 relation missing/invalid."
                )
            )
    ...
    @property
    def loki_worker_relations(self) -> List[ops.Relation]:
        """Returns the list of worker relations."""
        return self.model.relations.get("loki_worker", [])
    ...
    def _workers_scrape_jobs(self) -> List[Dict[str, Any]]:
        ...
        for worker in worker_topologies:
            job = {
                ...
                "relabel_configs": [
                    {"target_label": "juju_charm", "replacement": "loki-worker-k8s"},
    ...
    def _update_loki_cluster(self):  # common exit hook
        ...
        self.cluster_provider.publish_data(
            loki_config=self.coordinator.build_config(
    ...
    def has_multiple_workers(self) -> bool:
        """Return True if there are multiple workers forming the Loki cluster."""
        loki_cluster_relations = self.model.relations.get("loki-cluster", [])
        remote_units_count = sum(
            len(relation.units)
            for relation in loki_cluster_relations
            if relation.app != self.model.app
        )
        return remote_units_count > 1

    def publish_grant_secrets(self) -> None:
        """Publish and Grant secrets to the loki-cluster relation."""
        secrets = {
        ...
        relations = self.model.relations["loki-cluster"]
        for relation in relations:
